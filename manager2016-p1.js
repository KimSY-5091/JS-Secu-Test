...
managerJs.systemevent = function(idNm) {
	...
	var eventobj = {
		...
		'event' : {
			...
			'header-butt-license' : function event() {
				if (confirm('라이선스 신청을 하시겠습니까?')) {
					window.open('http://rep.solbitech.com/user');
				}
				return;
			},
...
managerJs.ajaxmainview = {
	...
	'list' : function() {
		...
		$('input[name=' + managerJs.ajaxmainview.obj.name + ']:checked').each(function() {
			...
			$('.' + infokey + '-msg').html(managerJs.scriptXss(managerJs.ajaxmainview.obj.msg));
		});
	...
	'ajax' : function() {
		...
		$.ajax({
			...
			error : function(e) {
				$('.' + managerJs.scriptXss(data['infoKey'])).css('color', '#000000');
				$('.' + managerJs.scriptXss(data['infoKey'])).html('[ERROR]');
				$('.' + managerJs.scriptXss(data['infoKey']) + '-msg').css('color', '#000000');
				$('.' + managerJs.scriptXss(data['infoKey']) + '-msg').html('▶ 통신중 에러가 발생하였습니다.');
				managerJs.ajaxmainview.count++;
				setTimeout('managerJs.ajaxmainview.ajax()', 1000);
			}
		});
	...
	var viewcontrol = {
		'main-view-report' : { // 샘플 리포트 호출
			'run' : function() {
				...
				var installPath = $('input:text[name="installPath"]').val() + 'manager/@REPORT.jsp';
				//installPath = installPath.replace('@REPORT', managerJs.scriptXss(obj.code));
				//$('.main-view-rpt-frame').attr('src', managerJs.scriptXss(installPath));
                installPath = installPath.replace('@REPORT', obj.code);
                $('.main-view-rpt-frame').attr('src', escapeInput(installPath));
		...
		'main-view-log' : { // 로그 정보 로드
			'run' : function() {
				$('#manager-main-view-lding').css('display', 'block');
				$('#' + managerJs.scriptXss(obj.code) + '-msg').html('로그를 불러오는 중입니다.');
				$.ajax({
					type : 'post',
					dataType : 'json',
					url : managerval.managerInfo,
					...
					success : function(jsondata) {
						managerJs.systemevent('monitor-loading-close');
						$('#' + managerJs.scriptXss(jsondata.id) + '-msg').html(managerJs.scriptXss(jsondata.message));
					},
					error : function(e) {
						alert(e);
					}
		...
		'main-view-back' : { // 파일 백업 정보 로드
			'run' : function() {
				$('#manager-main-view-lding').css('display', 'block');
				$('#' + managerJs.scriptXss(obj.code) + '-msg').html('모듈 백업을 요청하는 중입니다.');
				$.ajax({
					type : 'post',
					dataType : 'json',
					url : managerval.managerInfo,
		...
		'main-view-pro' : { // 프로퍼티 정보 로드
			'run' : function() {
				$('#manager-main-view-lding').css('display', 'block');
				$('#' + managerJs.scriptXss(obj.code) + '-msg').html('프로퍼티를 불러오는 중입니다.');
				var unipass = '';
	...
	var event = {
		'add' : function(objmap, codemap) {
			...
			if ($('#' + codemap.id).attr('id') == null) {
				...
				$(managerJs.scriptXss(codemap.savebox)).append(mapinput);
				var mapoption = document.createElement('option');
				mapoption.setAttribute('value', codemap.id);
				mapoption.innerHTML = codemap.id;
				$(managerJs.scriptXss(codemap.select)).append(managerJs.scriptXss(mapoption));
				$(managerJs.scriptXss(codemap.select)).val(managerJs.scriptXss(codemap.id)).attr('selected', 'selected');
			} else {
				$('#' + managerJs.scriptXss(codemap.id)).attr('value', managerJs.scriptXss(addmap));
			}
		},
...
$(document).ready(function() {
	...
	// 2018.01.03 서버종류 웹로직 여부 판단
	$('#WebApplicationServerInfo').change(function() {
		if ($(this).val() != 'weblogic') {
			$('#weblogicDomain').css('display', 'none');
			$('#weblogicPort').css('display', 'none');
		} else {
			$('#weblogicDomain').css('display', 'block');
			$('#weblogicPort').css('display', 'block');
		}
	});
	...
	// 서버패턴처리
	$('#convertServer').change(function() {
		managerJs.changediv($('#convertServer option:selected').val() != 'true', '#convertServerPt');
		managerJs.changediv($('#convertServer option:selected').val() != 'true', '#convertServerUrl');
	});
