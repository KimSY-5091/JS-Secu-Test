	...
	$(document).ready(function(){
		...
		$(window).resize(function(){
			...
			$('#report').css({
				...
				'width': function(){return report_width + report_widthUnit;},
                //height: 기동시와 동일하게 -2 추가 (고해상도 스크롤높이확보, 2025.7.23)
				'height': function(){return ($(window).height()-bottomMargine-2) + 'px';}
			});
		...
		$("#report").scroll(function(){
            ...
            // menuHeight+1: +5에서 변경(고해상도에서 Next미유발 패치, 2025.7.23)
            var bottomPosition = position + $(window).height() - menuHeight + 1;
    ...
    function goScrollAjax(jspUrl, parameter, method, pageNum){
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                // scrollTop: 10에서 3으로 일괄 변경 (고해상도에서 Next유발 높이확보, 2025.7.23)
                if (browser.indexOf("MSIE9") != -1) {
                    $('#report').animate({	scrollTop: 3 }, 0);
                }
                else if (browser.indexOf("Chrome") != -1) {
                    $('#report').animate({	scrollTop: 3 }, 0);
                }
                    ...
                    $('#report').animate({	scrollTop: 3 + 1}, 0);
                }
                else {
                ...
                $("#page option").eq(pageNum-1).prop("selected", "selected");
    ...
    function callRequestUrl(url, params, method) {
        ...
        var wholeHtml = "<html><head></head><body><form name='myform' method='" + method +
                        "' action='" + url + "'>" + formInnerHtml + "</form>" + "</body></html>";
